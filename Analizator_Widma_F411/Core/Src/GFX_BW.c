...
// funkcja rysujaca znak
void GFX_DrawChar(int x, int y, char chr, uint8_t color, uint8_t background)
{
	if(chr > 0x7E) return; // chr > '~';	Nie obsluguje ostatniego znaku ASCII i znakow spoza zakresu 8-bitowego

	// petla leci po kolumnach
	for(uint8_t i=0; i<font[1]; i++ ) // Each column (Width)	font[1] - liczba kolumn
	{
		// pobranie ustawienia pikseli z tablicy z fontami z font_8x5.h; (pobiera jedna kolumne)
		//			-0x20 => bo mamy czcionke dla znakow od 0x20 w gore
        uint8_t line = (uint8_t)font[(chr-0x20) * font[1] + i + 2]; // Take this line, (chr-0x20) = move 20 chars back,

        // petla leci po wierszach; line >>= 1   =>   przesuwa wiersz (line) w prawo
        for(int8_t j=0; j<font[0]; j++, line >>= 1) // For each pixel in column		font[0] - liczba wierszy
        {
            if(line & 1) // Check last pixel in line;		Czy piksel zapalony
            {
            	if(size == 1) // najmniejsza czcionka - "punktowa"
            		GFX_DrawPixel(x+i, y+j, color); // Draw this pixel
            	else // wieksza czcionka - rysowana prostokatami
            		GFX_DrawFillRectangle(x+i*size, y+j*size, size, size, color); // Or bigger pixel
            }
            else if(background == 0) // jesli piksel zgaszony i tlo nieprzezroczyste
            {
            	if(size == 1)
...
// funkcja rysujaca napis
void GFX_DrawString(int x, int y, char* str, uint8_t color, uint8_t background)
{
	int x_tmp = x; // "wskaznik"

	char znak;
	znak = *str; // pierwszy znak ze stringa

	while(*str++) // dopoki jest zawartosc stringa - dopoki nie doszedl do znaku konca '/0'
	{
		GFX_DrawChar(x_tmp, y, znak, color, background); // Draw current char	Rysuje znak

		// Przesuwa "wskaznik" w prawo 1 kolumne (spacja) + o szerokosc znaku (np. 5) * size (skalar powiekszajacy czcionke)
		x_tmp += ((uint8_t)font[1] * size) + 1; // Move X drawing pointer do char width + 1 (space)

		// czarna/biala ("widoczna") spacja - przydatne, jesli "wskaznik" natrafia na podswietlony wczesniej piksel
		...
		znak = *str; // Next char		Przesuwa wskaznik chodzacy po stringu
	}
}
#endif
#if USING_LINES == 1
